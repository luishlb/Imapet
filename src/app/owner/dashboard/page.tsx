...
async function fetchExames(): Promise<Exame[]> {
...
async function fetchDespesas(): Promise<Despesa[]> {
  const { data } = await createClient().from("despesas").select("id, data, valor, categoria").order("data", { ascending: false });
...
function AreaChart({ data }: { data: { label: string; total: number }[] }) {
  ...
  return (
    <svg ref={svgRef} viewBox={`0 0 ${VW} ${VH}`} className="w-full" style={{ height: 170 }}
      ...
      <path d={area} fill="url(#gradOwner)" />
...
export default function OwnerDashboardPage() {
  ...
  useEffect(() => {
    ...
    setAuth(true);
    Promise.all([fetchExames(), fetchDespesas()]).then(([ex, dep]) => {
      setExames(ex);
      setDespesas(dep);
      setCarregando(false);
    });
  ...
  const despesasFiltradas = useMemo(() => {
    return despesas.filter(d => {
      if (modo === "mes_atual") return d.data >= PRIMEIRO_DIA_MES && d.data <= HOJE_STR;
      if (modo === "mensal") {
        const mesStr = String(mesSel).padStart(2, "0");
        return d.data.startsWith(`${anoSel}-${mesStr}`);
      }
      return d.data >= dataInicioAplicada && d.data <= dataFimAplicada;
    });
  }, [despesas, modo, mesSel, anoSel, dataInicioAplicada, dataFimAplicada]);

  const fatBruto = useMemo(() => filtrados.reduce((s, e) => s + (e.valor_bruto || 0), 0), [filtrados]);
  const totalVet = useMemo(() => Math.round(fatBruto * 0.42 * 100) / 100, [fatBruto]);
  const fatEmpresa = useMemo(() => Math.round((fatBruto - totalVet) * 100) / 100, [fatBruto, totalVet]);
  const ticketMedio = useMemo(() => filtrados.length > 0 ? fatBruto / filtrados.length : 0, [fatBruto, filtrados]);
  const totalDespesas = useMemo(() => despesasFiltradas.reduce((s, d) => s + d.valor, 0), [despesasFiltradas]);
  const resultado = useMemo(() => fatEmpresa - totalDespesas, [fatEmpresa, totalDespesas]);
          ...
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="font-semibold text-text-main">Despesas no período</h2>
                <Link href="/owner/despesas" className="text-xs text-primary hover:underline">Ver todas →</Link>
              </div>
